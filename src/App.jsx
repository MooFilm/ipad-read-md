...
2. เลือกไฟล์ .md, .txt, .pdf หรือ .docx จากเครื่องของคุณ
...
function stripExtension(name) {
  return name.replace(/\.(md|markdown|txt|pdf|docx)$/i, '')
}
...
async function extractBookContent(file) {
  const extension = getFileExtension(file.name)

  if (extension === 'pdf') {
    return {
      content: await extractPdfText(file),
      typeLabel: FILE_TYPE_LABEL.pdf,
    }
  }
  ...
  if (extension === 'docx') {
    ...
    return {
      ...
      typeLabel: FILE_TYPE_LABEL.docx,
    }
  }
  ...
  if (extension === 'md' || extension === 'markdown' || extension === 'txt') {
    return {
      ...
      typeLabel: FILE_TYPE_LABEL[extension],
    }
  }
  ...
  async function handleImportFiles(event) {
    ...
    const resultItems = await Promise.all(
      files.map(async (file) => {
        try {
          const { content, typeLabel, unsupported } = await extractBookContent(file)
    ...
    const incomingBooks = resultItems
      .filter((item) => item.type === 'book')
      .map((item) => item.book)
    const unsupportedCount = resultItems.filter((item) => item.type === 'unsupported').length
    const emptyCount = resultItems.filter((item) => item.type === 'empty').length
    const errorCount = resultItems.filter((item) => item.type === 'error').length
    ...
    if (unsupportedCount) {
      showToast(`ข้าม ${unsupportedCount} ไฟล์ที่ยังไม่รองรับ (รองรับ .md .markdown .txt .pdf .docx)`)
    }

    if (emptyCount) {
      showToast(`ข้าม ${emptyCount} ไฟล์ที่ไม่มีข้อความให้อ่าน`)
    }

    if (errorCount) {
      showToast(`ข้าม ${errorCount} ไฟล์ที่อ่านไม่สำเร็จ`)
    }
          ...
          <section className="book-section">
            ...
            {isBooting ? (
              ...
              <div className="placeholder-card">
                โฟลเดอร์นี้ยังไม่มีหนังสือ กด “เพิ่มหนังสือ” เพื่อ import ไฟล์ `.md`, `.txt`, `.pdf` หรือ `.docx`
              </div>
            )}
          </section>
          ...
          <div className="modal-card help-card" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="help-list">
              ...
              <article>
                ...
                <p>กด “เพิ่มหนังสือ” แล้วเลือกไฟล์ .md, .markdown, .txt, .pdf หรือ .docx จากเครื่องของคุณ ข้อมูลจะอยู่ในเครื่องนี้เท่านั้น</p>
              </article>
              <article>
