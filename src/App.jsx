...
function App() {
  ...
  const [ghSettingsDraft, setGhSettingsDraft] = useState({
    ...
    uploadEnabled: defaultPrefs.github.uploadEnabled,
  })
  ...
  const githubUploadRequested = Boolean(githubPrefs.uploadEnabled)
  const githubUploadReady = githubUploadRequested && Boolean(githubToken)
  ...
  function openGhSettings() {
    setGhSettingsDraft({
      ...
      uploadEnabled: githubPrefs.uploadEnabled ?? defaultPrefs.github.uploadEnabled,
    })
    ...
    if (repoInput) {
      ...
      const nextPrefs = normalizePrefs({
        ...
        github: {
          ...
          uploadEnabled: ghSettingsDraft.uploadEnabled,
          configApplied: true,
        },
      })
  ...
  const triggerAutoSync = useCallback(() => {
    ...
    const now = Date.now()
    const lastAttempt = lastAutoSyncAtRef.current
    const lastSynced = githubPrefs.lastSyncedAt ?? 0

    if (now - Math.max(lastAttempt, lastSynced) < AUTO_SYNC_MIN_INTERVAL) {
      return
    }

    lastAutoSyncAtRef.current = now
    void handleGithubSync({ silent: true })
  }, [githubPrefs.syncMode, githubPrefs.lastSyncedAt, githubRepoKey, ghSyncing, handleGithubSync, isBooting])
  ...
  useEffect(() => {
    ...
    triggerAutoSync()
  }, [githubPrefs.syncMode, githubRepoKey, triggerAutoSync, isBooting])
  ...
  }, [githubPrefs.syncMode, githubRepoKey, triggerAutoSync])
          ...
          <form className="modal-card gh-modal gh-settings" onClick={(event) => event.stopPropagation()} onSubmit={handleSaveGhSettings}>
            ...
            <div className="gh-field-group">
              ...
              <input
                ...
                placeholder="ghp_xxxxxxxxxxxx"
              />
              <p className="gh-token-note">ใส่ token เฉพาะเมื่อจะอัปโหลดไฟล์ หรือเมื่อ repo เป็น private</p>
            </div>

            <div className="gh-field-group">
              <label className="gh-label">โหมดอัปโหลด</label>
              <label className="gh-toggle">
                <input
                  type="checkbox"
                  checked={ghSettingsDraft.uploadEnabled}
                  onChange={(event) =>
                    setGhSettingsDraft((current) => ({ ...current, uploadEnabled: event.target.checked }))
                  }
                />
                <span>อัปโหลดไฟล์เมื่อเพิ่มหนังสือจาก iPad</span>
              </label>
              <p className="gh-token-note">ถ้าปิดไว้ ระบบจะซิงก์แบบอ่านอย่างเดียว</p>
            </div>
            ...
            <div className="gh-field-group">
              ...
              >
                ...
                <option value="auto">ซิงก์อัตโนมัติเมื่อเปิดแอปหรือกลับมาใช้งาน</option>
              </select>
            </div>
