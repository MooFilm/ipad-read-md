...
- หนังสือที่คุณ import, โฟลเดอร์, progress, และ bookmark จะเก็บอยู่ในเบราว์เซอร์ของเครื่องนี้เป็นหลัก
- ถ้าต้องการเปิดจากเครื่องอื่น ให้ใช้เมนู GitHub Cloud เพื่ออัปโหลด/ดึงแบ็กอัป
...
async function upsertGithubFile(owner, repo, filePath, content, token) {
  const headers = { Accept: 'application/vnd.github+json' }
  if (token) headers.Authorization = `Bearer ${token}`

  const encodedPath = encodeGithubPath(filePath)
  const metaRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${encodedPath}`, { headers })
  let sha
  ...
  if (metaRes.ok) {
    ...
    throw new Error(`ตรวจสอบไฟล์บน GitHub ไม่สำเร็จ (${metaRes.status})`)
  }

  const payload = {
    message: `Update ReadShelf backup (${new Date().toISOString().slice(0, 10)})`,
  ...
  if (!putRes.ok) {
    ...
    throw new Error(`อัปโหลดขึ้น GitHub ไม่สำเร็จ (${putRes.status})${message}`)
  }
...
function App() {
  ...
  const [ghCloudOpen, setGhCloudOpen] = useState(false)
  const [ghCloudRepo, setGhCloudRepo] = useState('')
  const [ghCloudPath, setGhCloudPath] = useState(defaultPrefs.ghBackupPath)
  const [ghCloudToken, setGhCloudToken] = useState('')
  const [ghCloudStatus, setGhCloudStatus] = useState('idle')
  const [ghCloudError, setGhCloudError] = useState('')
  ...
  const ghCloudBusy = ghCloudStatus === 'uploading' || ghCloudStatus === 'downloading'
  ...
  async function applyBackupPayload(payload, extraPrefs = {}) {
  ...
  function openGhCloudModal() {
    setGhCloudOpen(true)
    setGhCloudRepo(prefs.ghBackupRepo ?? '')
    setGhCloudPath(prefs.ghBackupPath ?? defaultPrefs.ghBackupPath)
    setGhCloudToken('')
    setGhCloudStatus('idle')
    setGhCloudError('')
  }

  function closeGhCloudModal() {
    setGhCloudOpen(false)
    setGhCloudRepo('')
    setGhCloudPath(defaultPrefs.ghBackupPath)
    setGhCloudToken('')
    setGhCloudStatus('idle')
    setGhCloudError('')
  }
  ...
  async function handleGithubBackupUpload() {
    const parsed = parseGithubRepo(ghCloudRepo)
    if (!parsed) {
      setGhCloudError('รูปแบบไม่ถูกต้อง ตัวอย่าง: owner/repo หรือ https://github.com/owner/repo')
      return
    }

    const filePath = normalizeGithubPath(ghCloudPath)
    const token = ghCloudToken.trim()
    setGhCloudStatus('uploading')
    setGhCloudError('')
    ...
    try {
      ...
      const nextPrefs = {
        ...
        lastBackupAt: exportedAt,
        lastBackupReminderDismissedAt: exportedAt,
        ghBackupRepo: `${parsed.owner}/${parsed.repo}`,
        ghBackupPath: filePath,
      }
      const payload = createBackupPayload(folders, books, nextPrefs, exportedAt)
      const payloadText = JSON.stringify(payload, null, 2)

      await upsertGithubFile(parsed.owner, parsed.repo, filePath, payloadText, token)

      setPrefs(nextPrefs)
      await putPrefs(nextPrefs)
      showToast('อัปโหลดแบ็กอัปขึ้น GitHub แล้ว')
      setStatus('สำรองข้อมูลขึ้น GitHub Cloud เรียบร้อยแล้ว')
      setGhCloudStatus('idle')
      closeGhCloudModal()
    } catch (error) {
      setGhCloudStatus('error')
      setGhCloudError(String(error.message ?? error))
    }
  }

  async function handleGithubBackupDownload() {
    const parsed = parseGithubRepo(ghCloudRepo)
    if (!parsed) {
      setGhCloudError('รูปแบบไม่ถูกต้อง ตัวอย่าง: owner/repo หรือ https://github.com/owner/repo')
      return
    ...
    const filePath = normalizeGithubPath(ghCloudPath)
    const token = ghCloudToken.trim()
    setGhCloudStatus('downloading')
    setGhCloudError('')

    try {
      const payloadText = await fetchGithubFileContent(parsed.owner, parsed.repo, filePath, token)
      const payload = JSON.parse(payloadText)

      if (!window.confirm('นำเข้าแบ็กอัปจาก GitHub และแทนที่คลังหนังสือทั้งหมดหรือไม่?')) {
        setGhCloudStatus('idle')
        return
      }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="library-stack">
        <section className="library-panel">
          ...
          <section className="library-bottom-actions">
            ...
            <button type="button" className="ghost-button" onClick={openGhCloudModal}>
              GitHub Cloud
            </button>
          ...
          <div className="modal-card help-card" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="help-list">
              ...
              <article>
                ...
                <p>กด “เพิ่มหนังสือ” แล้วเลือกไฟล์ .md จากเครื่องของคุณ ข้อมูลจะอยู่ในเครื่องนี้เป็นหลัก</p>
              </article>
              <article>
              ...
              <article>
                ...
                <p>ใช้ปุ่ม “GitHub Cloud” เพื่ออัปโหลดแบ็กอัปไปยัง repo แล้วดึงกลับมาบนอุปกรณ์อื่น</p>
              </article>
              <article>
      ...
      {ghCloudOpen ? (
        <div className="modal-overlay" onClick={closeGhCloudModal}>
          <div className="modal-card gh-modal" onClick={(event) => event.stopPropagation()}>
            <p className="eyebrow">GitHub Cloud</p>
            <h3>สำรองคลังขึ้น GitHub</h3>
            ...
            <div className="gh-field-group">
              ...
              <input
                ...
                className="gh-input"
                value={ghCloudRepo}
                onChange={(event) => setGhCloudRepo(event.target.value)}
                placeholder="owner/repo หรือ https://github.com/owner/repo"
                disabled={ghCloudBusy}
              />
            </div>

            <div className="gh-field-group">
              <label className="gh-label">ไฟล์แบ็กอัปใน repo</label>
              <input
                type="text"
                className="gh-input"
                value={ghCloudPath}
                onChange={(event) => setGhCloudPath(event.target.value)}
                placeholder="readshelf-backup.json"
                disabled={ghCloudBusy}
              />
            </div>
            ...
            <div className="gh-field-group">
              <label className="gh-label">
                Personal Access Token <span className="gh-optional">(ต้องใช้สิทธิ์เขียน repo)</span>
              </label>
              <input
                type="password"
                className="gh-input"
                value={ghCloudToken}
                onChange={(event) => setGhCloudToken(event.target.value)}
                placeholder="ghp_xxxxxxxxxxxx"
                disabled={ghCloudBusy}
              />
              <p className="gh-token-note">Token จะไม่ถูกบันทึกไว้ในเครื่อง ใช้เฉพาะครั้งนี้เท่านั้น</p>
            ...
            <p className="gh-token-note">ระบบจะสร้างไฟล์แบ็กอัป JSON ใน repo เพื่อเปิดจากอุปกรณ์อื่นได้</p>

            {ghCloudError ? <p className="gh-error">{ghCloudError}</p> : null}

            <div className="modal-actions">
              <button type="button" className="ghost-button" onClick={closeGhCloudModal} disabled={ghCloudBusy}>
                ยกเลิก
              </button>
              <button
                type="button"
                className="ghost-button"
                onClick={handleGithubBackupDownload}
                disabled={ghCloudBusy || !ghCloudRepo.trim() || !ghCloudPath.trim()}
              >
                {ghCloudStatus === 'downloading' ? 'กำลังดึงแบ็กอัป...' : 'ดึงแบ็กอัป'}
              </button>
              <button
                type="button"
                className="primary-button"
                onClick={handleGithubBackupUpload}
                disabled={ghCloudBusy || !ghCloudRepo.trim() || !ghCloudPath.trim()}
              >
                {ghCloudStatus === 'uploading' ? 'กำลังอัปโหลด...' : 'อัปโหลดขึ้น GitHub'}
              </button>
            </div>
          </div>
        </div>
